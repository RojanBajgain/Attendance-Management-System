...
// Improved logo loading function
Future<pw.ImageProvider?> _loadLogo(ThemeService themeService) async {
  try {
    if (themeService.logoUrl.isNotEmpty) {
      print('Loading logo from URL: ${themeService.logoUrl}');

      // Load network image with timeout
      ...
      if (response.statusCode == 200) {
        ...
        if (_isValidImageFormat(imageBytes)) {
          ...
          print('Invalid image format from network URL');
        }
      } else {
        print('Failed to load network image: Status ${response.statusCode}');
      }
    }
  } catch (e) {
  ...
  try {
    print('Loading fallback asset image');
    final ByteData logoData = await rootBundle.load(AppImages.appLogoHR);
    final assetBytes = logoData.buffer.asUint8List();

    if (_isValidImageFormat(assetBytes)) {
      return pw.MemoryImage(assetBytes);
    } else {
      print('Invalid format for asset image');
    }
  } catch (e) {
...
Future<void> generateAndSavePDF(
  ...
  final logoProvider = await _loadLogo(themeService);
            ...
            children: [
              ...
              pw.Row(
                ...
                children: [
                  ...
                  pw.Spacer(),
                  // Only show logo if it loaded successfully
